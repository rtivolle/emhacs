...
Also creates status sensors for Emporia EV Chargers so you can see charger state, whether it's on, and its current/max charging rate from Home Assistant.
...
- Emporia EV chargers are exposed as status sensors with attributes for charger on/off, message, and charging rate.
